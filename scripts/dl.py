...
def progress(dl):
    try:
        while True:
            sleep(1)
            print(f"Progress: {dl.completed_length_string()}/{dl.total_length_string()} ({dl.progress_string()}), Speed: {dl.download_speed_string()}, ETA: {dl.eta_string()}            ", end="\r", flush=True)
            dl.update()
            if dl.is_complete:
                break
        ...
        print("\nStopping download...")
        dl.pause()
        to_del = True if input("Do you want to delete the incomplete file as well? (y/n): ") == "y" else False
...
def download(url, downloads_dir):
    ...
    for category, exts in categories.items():
        if ext in exts:
            shutil.move(f"{downloads_base_path}/{filename}", f"{downloads_dir}/{category}/{filename}")
            break
    else:
...
def download_torrent_from_libgen(page_url, downloads_dir):
    ...
    title_td = next(td for td in soup.select('td') if td.string and 'Title:' in td.string)
    title = title_td.find_next('b').string.strip()

    # Locate the <td> whose .string contains "Author(s):"
    authors_td = next(td for td in soup.select('td') if td.string and 'Author(s):' in td.string)
    authors = authors_td.find_next('b').string.strip()

    # Find the Torrent per 1000 files link
    torrent_link = libgen_base_url + soup.find('a', href=lambda href: href and "repository_torrent" in href and ".torrent" in href)['href'][2:]

    result = subprocess.run(f"curl -o torrent.torrent {torrent_link} && aria2c --show-files torrent.torrent", capture_output=True, text=True, shell=True)
    ...
    dl = aria2.add("./torrent.torrent", options={
        "seed-time": "0",
